...
const DEFAULT_FIXED_CATEGORIES = [
  { key: "rent", label: "임대료", removable: false },
  { key: "depreciation", label: "감가상각비 (자동반영)", removable: false },
  { key: "employeeSalary", label: "정직원 인건비", removable: false },
  { key: "retirementFund", label: "퇴직적립금", removable: true },
  { key: "welfare", label: "복리후생비", removable: true },
  { key: "insurance", label: "4대보험 등", removable: true },
  { key: "phone", label: "전화요금", removable: true },
  { key: "posRental", label: "포스기임대료", removable: true },
  { key: "security", label: "보안업체이용료", removable: true },
  { key: "internet", label: "인터넷사용료", removable: true },
  ...
  { key: "rental", label: "정수기 등 기타렌탈료", removable: true },
  { key: "tax", label: "세무사비용 및 기타 세금", removable: true },
]
...
const DEFAULT_VARIABLE_CATEGORIES = [
  ...
  { key: "vat", label: "부가가치세 (매출 자동반영)", removable: false, isCalculated: true },
]
...
export default function RestaurantBEPCalculator() {
  ...
  const handleNewFixedCostAmountChange = (value: string) => {
    ...
    if (newFixedCostItem.category === "employeeSalary" && amount > 0) {
      autoRetirementFundDisplay = `퇴직금(9.6%): ${formatNumberForDisplay(amount * 0.096, "원")}`
      autoInsuranceDisplay = `4대보험(9%): ${formatNumberForDisplay(amount * 0.09, "원")}`
    }
  ...
  const handleEditCostAmountChange = (value: string) => {
    ...
    const amount = parseInputAsNumber(value) * 1000 // Input is 천원, store as actual
    ...
    if (editingCostCategory === "employeeSalary" && amount > 0) {
      autoRetirementFundDisplay = `퇴직금(9.6%): ${formatNumberForDisplay(amount * 0.096, "원")}`
      autoInsuranceDisplay = `4대보험(9%): ${formatNumberForDisplay(amount * 0.09, "원")}`
    }
  ...
  const addFixedCostItem = () => {
    ...
    const amountValue = parseInputAsNumber(newFixedCostItem.amount) // Already in actual value
    ...
    if (newFixedCostItem.category === "employeeSalary" && amountValue > 0) {
      const retirement = amountValue * 0.096
      const insurance = amountValue * 0.09
      item.memo =
        `${item.memo || ""} (참고: 퇴직금 약 ${formatNumberForDisplay(retirement, "원")}, 4대보험 약 ${formatNumberForDisplay(insurance, "원")})`.trim()
    }
    setFixedCosts((prev) => ({
      ...prev,
  ...
  const startEditCost = (item: CostDetailItem, category: string, type: "fixed" | "variable") => {
    ...
    // Populate autoRetirementFundDisplay and autoInsuranceDisplay for employeeSalary when starting edit
    let autoRetirementFundDisplay = ""
    let autoInsuranceDisplay = ""
    if (type === "fixed" && category === "employeeSalary" && item.amount > 0) {
      autoRetirementFundDisplay = `퇴직금(9.6%): ${formatNumberForDisplay(item.amount * 0.096, "원")}`
      autoInsuranceDisplay = `4대보험(9%): ${formatNumberForDisplay(item.amount * 0.09, "원")}`
    }
  ...
  const saveEditCost = () => {
    ...
    // Amount is already actual value from handleEditCostAmountChange or handleEditCostPartTimeChange
    if (editingCostType === "fixed" && editingCostCategory === "employeeSalary" && finalCostItem.amount > 0) {
      const retirement = finalCostItem.amount * 0.096
      const insurance = finalCostItem.amount * 0.09
      const baseMemo = finalCostItem.memo?.split(" (참고:")[0] || ""
      finalCostItem.memo =
        `${baseMemo} (참고: 퇴직금 약 ${formatNumberForDisplay(retirement, "원")}, 4대보험 약 ${formatNumberForDisplay(insurance, "원")})`.trim()
      ...
      if (dailyWage > 0 && workDays > 0) {
        finalCostItem.amount = dailyWage * workDays // Ensure amount is correct
  ...
  const getTotalVariableCosts = useCallback((): number => {
    const generalVariableCosts =
      DEFAULT_VARIABLE_CATEGORIES.filter((cat) => !cat.isCalculated && !cat.isCardFee).reduce(
        (total, cat) => total + getVariableCategoryTotal(cat.key),
        0,
  ...
  }, [variableCosts, specificCardFees, customVariableCategories, getVariableCategoryTotal])
  ...
  const calculateDailySalesData = () => {
    ...
    const otherRev = parseInputAsNumber(otherRevenue) || 0 // Use parseInputAsNumber for safety
    const otherCost = parseInputAsNumber(otherCosts) || 0 // Use parseInputAsNumber for safety
  ...
  const calculateBEP = useCallback(() => {
    ...
    const variableCostsValue = getTotalVariableCosts() // This is other variable costs + card fees
    const marketingCostsValue = getCurrentMonthMarketingCosts()

    // Calculate average margin rate based on current menu items
    let totalProjectedRevenue = 0
    let totalProjectedMenuCost = 0 // Only cost of goods sold for menu items
    menuItems.forEach((item) => {
      totalProjectedRevenue += item.price // Use non-discounted price for general BEP
      totalProjectedMenuCost += item.cost
    })

    // Contribution Margin Rate = (Sales - Variable COGS) / Sales
    // For BEP, we need variable costs that scale directly with sales (i.e., menu costs)
    // The `averageMarginRate` should reflect the margin *after* direct menu costs.
    const contributionMarginRate =
      totalProjectedRevenue > 0 ? (totalProjectedRevenue - totalProjectedMenuCost) / totalProjectedRevenue : 0

    // BEP (Sales Revenue) = (Total Fixed Costs + Other Fixed-like Variable Costs + Marketing Costs) / Contribution Margin Rate
    // "Other Fixed-like Variable Costs" are those from the variableCosts state (electricity, gas, etc.) plus card fees,
    // as these are not directly tied to individual menu item sales in the same way COGS is for margin calculation.
    ...
    return {
      ...
      monthlyVariableCosts: variableCostsValue, // This is other variable costs + card fees
      averageMarginRate: contributionMarginRate * 100, // This is actually contribution margin rate
      operatingDays,
      dailyBEP,
      monthlyBEP,
    }
  ...
  const getMonthlyAnalysis = useCallback(() => {
    ...
    const totalMenuCostsFromSales = monthSales.reduce((sum, sale) => {
      // Recalculate menu costs based on actual sales and menu item costs
      let dailyMenuCost = 0
      ...
      return sum + dailyMenuCost + sale.otherCosts // Add other daily costs
    }, 0)

    const totalFixedCostsForMonth = getTotalFixedCosts()
    const otherMonthlyVariableCosts = Object.keys(variableCosts)
      .filter((cat) => !["ingredients", "cardFees", "vat"].includes(cat)) // ingredients are part of totalMenuCostsFromSales
      .reduce((total, category) => total + getVariableCategoryTotal(category), 0)
    const totalCardFees = getVariableCategoryTotal("cardFees")
    const vatPayable =
      vatSettings.enabled && vatSettings.autoCalculate ? calculateAutoVAT() : getVariableCategoryTotal("vat")
    ...
    return {
      ...
      totalMarketingCosts: totalMarketingCostsForMonth,
  ...
  const getCostOptimizationSuggestions = () => {
    ...
    if (monthlyAnalysisData.variableCostRate > 40) {
      // This rate now includes COGS, other variable, card fees, VAT
      suggestions.push({
        category: "변동비 절감",
        priority: "중간",
  ...
  const currentDaySalesData = calculateDailySalesData() // Ensure this is called to reflect current inputs
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-2">
          {isEditingRestaurantName ? (
            ...
            <h1
              className="text-2xl sm:text-3xl font-bold cursor-pointer hover:text-blue-600"
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          <Button onClick={exportToExcel} className="flex items-center gap-2 text-xs sm:text-sm">
            <Download className="h-4 w-4" />
            메뉴 CSV
          </Button>
          <Button onClick={exportAllData} variant="outline" className="flex items-center gap-2 text-xs sm:text-sm">
            <Download className="h-4 w-4" />
            전체 데이터(JSON)
          </Button>
          <div>
          ...
          <Button onClick={saveData} className="flex items-center gap-2 text-xs sm:text-sm">
      ...
      <Tabs defaultValue="menu" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6 text-sm">
              <div className="p-4 border rounded-lg bg-gray-50 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      id="dep-amount"
                      type="number"
                      value={newDepreciationItem.investmentAmount}
                      onChange={(e) =>
                        setNewDepreciationItem({ ...newDepreciationItem, investmentAmount: e.target.value })
                      }
                      placeholder="예: 5000"
                      step="0.1"
                    />
                ...
                <Button onClick={addDepreciationItem} className="w-full sm:w-auto">
                  <Plus className="h-4 w-4 mr-2" />
                  감가상각 항목 추가
                </Button>
              </div>
        ...
        <TabsContent value="costs">
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="p-3 border rounded-lg bg-gray-50 space-y-2">
                  ...
                  <Input
                    ...
                    value={
                      newFixedCostItem.amount ? (parseInputAsNumber(newFixedCostItem.amount) / 1000).toString() : ""
                    }
                    onChange={(e) => handleNewFixedCostAmountChange(e.target.value)}
                    step="0.1"
                  />
                  {newFixedCostItem.category === "employeeSalary" && (
                    <div className="text-xs text-blue-600 space-y-1 mt-1">
                      ...
                      <p>위 금액은 참고용이며, 실제 퇴직적립금 및 4대보험은 해당 카테고리에 별도로 입력해주세요.</p>
                    </div>
                  )}
                  <Input
                    ...
                    .map((categoryKey) => {
                      ...
                      return (
                        (items.length > 0 ||
                          ...
                          categoryKey !== "depreciation") && ( // Show category if it has items, or is depreciation with auto value, or is not depreciation (to allow adding to empty custom cats)
                              ...
                              editCostItem ? (
                                ...
                                >
                                  ...
                                  <Input
                                    type="number"
                                    placeholder="금액(천원)"
                                    value={editCostItem.amount / 1000}
                                    onChange={(e) => handleEditCostAmountChange(e.target.value)}
                                    step="0.1"
                                  />
                                  {categoryKey === "employeeSalary" && (
                                    <div className="text-xs text-blue-600 space-y-1 mt-1">
            ...
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="p-3 border rounded-lg bg-gray-50 space-y-2\">
                  ...
                  >
                    <option value="">카테고리 선택</option>
                    {DEFAULT_VARIABLE_CATEGORIES.filter((cat) => !cat.isCalculated && !cat.isCardFee).map((cat) => (
                      <option key={cat.key} value={cat.key}>
                        {cat.label}
                      </option>
                    ))}
                  ...
                  {newVariableCostItem.category === "partTime" ? (
                    ...
                    <Input
                      type="number"
                      placeholder="금액(천원)"
                      value={newVariableCostItem.amount}
                      onChange={(e) => setNewVariableCostItem((prev) => ({ ...prev, amount: e.target.value }))}
                      step="0.1"
                    />
                  )}
                  <Input
                ...
                {/* 변동비 목록 표시 */}
                <div className="space-y-3">
                  {DEFAULT_VARIABLE_CATEGORIES.filter((c) => !c.isCalculated && !c.isCardFee)
                    .map((cat) => cat.key)
                    ...
                    .map((categoryKey) => {
                      ...
                      return (
                        (items.length > 0 ||
                          (categoryKey !== "ingredients" && categoryKey !== "cardFees" && categoryKey !== "vat")) && ( // Show category if it has items or is a manually addable category
                          <div key={categoryKey} className="pt-2">
                            <div className="flex justify-between items-center mb-1">
                              <h5 className="font-semibold text-gray-700">{categoryLabel}</h5>
                ...
                <div className="p-3 bg-orange-50 rounded-lg mt-4">
                  ...
                  <div className="text-xs text-gray-500">(부가가치세 제외, 식재료비는 일일매출에서 반영)</div>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="mt-6">
            ...
            <CardContent className="flex flex-col sm:flex-row gap-3 items-end text-sm">
              <div className="flex-1">
              ...
              <Button onClick={addCustomCategory} className="w-full sm:w-auto">
                <Plus className="h-4 w-4 mr-2" />
                카테고리 추가
              </Button>
            </CardContent>
          ...
          <Card className="mt-6">
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="p-3 border rounded-lg bg-gray-50 space-y-2">
                ...
                />
                <Button onClick={addMarketingCost} size="sm" className="w-full">
                  <Plus className="h-4 w-4 mr-1" />
                  추가
                </Button>
              </div>
                  ...
                  {daysInMonth.map((day, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-1.5 min-h-[6.5rem] border rounded-md flex flex-col items-center justify-between text-xs transition-colors ${isSelectedDay ? "bg-blue-100 border-blue-300 ring-2 ring-blue-400" : sale ? (sale.bepAchieved ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200") : "hover:bg-gray-100"}`}
                      >
                        <span className="font-medium text-sm">{dayStr}</span>
                        {sale && (
            ...
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                  ...
                  {menuItems.map((menu) => (
                    <div key={menu.id} className="grid grid-cols-2 items-center gap-2">
                ...
                <table className="w-full min-w-[600px] border-collapse border border-gray-300 text-xs sm:text-sm">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="p-1.5 border text-left">메뉴명</th>
                      <th className="p-1.5 border">총 판매량</th>
                      <th className="p-1.5 border">총 매출액</th>
                      <th className="p-1.5 border">총 이익액</th>
                      <th className="p-1.5 border">일평균 판매량</th>
                      <th className="p-1.5 border">이익률</th>
                    </tr>
                  </thead>
                  <tbody>
                    {getMenuAnalysis()
                      ...
                      .map((analysis) => {
                        ...
                        return (
                          <tr key={analysis.menuId} className="text-center hover:bg-gray-50">
                            <td className="p-1.5 border font-medium text-left">{menu.name}</td>
                            <td className="p-1.5 border">{analysis.totalSold}</td>
                            <td className="p-1.5 border">{formatNumberForDisplay(analysis.totalRevenue, "원")}</td>
                            <td className="p-1.5 border">
                              <span className={analysis.totalProfit >= 0 ? "text-green-600" : "text-red-600"}>
                                {formatNumberForDisplay(analysis.totalProfit, "원")}
                              </span>
                            </td>
                            <td className="p-1.5 border">{analysis.averageDaily.toFixed(1)}</td>
                            <td className="p-1.5 border">
                              <Badge
                                variant={
                ...
                {vatSettings.enabled && (
                  <>
                    ...
                    {!vatSettings.autoCalculate && (
                      <p className="text-xs text-gray-500">
                        자동계산을 비활성화하면, 변동비의 '부가가치세' 항목에 직접 입력한 금액이 사용됩니다.
                      </p>
                    )}
                  </>
                )}
